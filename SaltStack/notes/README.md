...
```
Тут если приходит миньон, который заканчивается на *-red, то его сразу реджектит. В целом - на всё. Если что - тут удаляются только ключи (salt-key)

Если проверка тетовая, то нужно будет обновить конфиг для мастера с тем, чтобы были закомменчены реакты для глушения ключей красных ВМок.После ребута мастера - все ключи должны придти. Затем вводим команду: 
```bash
salt-key -A 
```
Она разрешит ключи миньонов в мастере

## Шедулнг

* Оно походе на крон или systemd.timer или на sheduler в кубе, если угодно
* Прописывается в конфиге миньона или мастера  
* Запуск моделей применение состояний
* На миньоне модули исполнения, на мастере - модули раннеров

Примерный конфиг шедулера: 
```sls
shedule:
  write_to_file:
    function: cmd.run
