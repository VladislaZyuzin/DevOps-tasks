...
```
    runs-on: ubuntu-latest
```
* Указывает, на какой операционной системе будет выполняться job. В данном случае используется версия ubuntu-latest, то есть - последняя.
...
5.  **ОС для выполнения `job`:**
    ...
    *   Указывает, на какой операционной системе будет выполняться job. В данном случае используется версия `Ubuntu-22.04`.
...
![image](https://github.com/user-attachments/assets/ad9d9dad-490f-4521-ada5-adf1900ba1ec)



## Что поменялось из плохого в хорошее: 
| Плохая практика в "Bad CI"                     | Исправление в "Good CI"                                                                 |
|------------------------------------------------|-----------------------------------------------------------------------------------------|
| Триггер на все пуши (`on: [push]`)             | Ограничение на ветку `main` и пул-реквесты в `main`.                                    |
| Ручная установка Python (`apt-get`)            | Использование `actions/setup-python@v2` для установки Python.                          |
| Отсутствие обновления `pip`                    | Добавлен шаг для обновления `pip` перед установкой зависимостей.                        |
| Деплой без проверки успешности тестов          | Удален шаг деплоя, добавлено уведомление об успешном завершении.                        |
| Использование `ubuntu-latest`                  | Использование конкретной версии Ubuntu (`ubuntu-24.04`).                                |
