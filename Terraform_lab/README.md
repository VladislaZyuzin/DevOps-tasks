...
## Финальная проверка
Для того, чтобы до конца удостовериться в выполненной работе введём команду: 
```bash
ssh ubuntu@<external_ip>  # Используйте IP из вывода terraform
```
Таким образом получтся зайти в систему. Скрин с пруфом: 

![image](https://github.com/user-attachments/assets/04f7b25d-7dff-466f-b869-ade4f2c14286)

Далее проверим отображаются ли виртуалки в системе, для этого введём команду: 
```bash
yc compute instance list
```
Так же введём команду для проверки подсетей:
```bash
yc vpc subnet list    
```
Если всё сделано правильно, то в качестве ответа получится такое сообщение:
![image](https://github.com/user-attachments/assets/05729f64-fdc4-4018-84d9-9f6bc87192a8)

# Заключение

В ходе работы была создана ВМ, 2 сети и 4 подсети. Были получены знания о таком важном инструменте IaC, как Terraform. 
